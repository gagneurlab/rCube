...
#' sample-specific size-factors by three different types. The 'spikeinGLM' method
#' fits a GLM to spike-in read counts to extract sequencing depth values and 
#' cross-contamination rates for all samples.
#' 
#' TODO: explain other methods 
#' 
#' Additional informations for the fit can be found in the metadata of the resulting
#' \code{rCubeExperiment}.
#'
...
#'
#' @return An updated \code{rCubeExperiment} object for features with sequencing depth 
#' values included in \code{colData} and updated metadata information.
#' 
...
estimateSizeFactors <- function(featureCounts, spikeinCounts, 
    ...
    if(method == "spikeinGLM"){
        featureCounts <- .calculateNormalizationBySpikeinGLM(featureCounts, spikeinCounts)
...
.calculateNormalizationBySpikeinGLM <- function(featureCounts, spikeinCounts){
    ...
    resultsList = list("sequencing.depth"=sequencingDepths,
                       ...
                       "intercept"=intercept,
                       "fittedCounts"=matrix(fittedCounts,nrow=length(spikeinSpecificBias)))
