...
Example data sets for a T cell activation are stored in the \texttt{inst/extdata} of the \textit{rCube} package.
...
expDesign
#setupExperiment(rows, designMatrix=expDesign, files=NULL)
@

Alternatively, the experimental design matrix can be constructed from the bam file names, if they follow the following convention
\texttt{\{condition\}\_\{L|T\}\_\{labelingTime\}\_\{replicate\}.bam}

%TODO; should we export: .createDesignMatrix

<<ExperimentalDesignFromFiles, echo=TRUE>>==
bamfiles <- list.files(folder, pattern="*.bam$")
bamfiles
#setupExperiment(rows, designMatrix=NULL, files=bamfiles)
@
