...
<<CountingTimeSeriesExample, echo=TRUE, eval=TRUE>>==
...
<<spikeincountsfilename2, echo=TRUE>>=
colData(spikeinCounts)$filename <- paste(folder, colData(spikeinCounts)$filename, sep="/")
@
...
repliable estimates from highly covered junctions. This is not reasonable for our
test set due to the downsampling. In general, filtering could be obtained via:
<<Filtering, echo=TRUE, eval=FALSE>>==
